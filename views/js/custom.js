	 ...
	 // AJAX POPUP
		...
		// BLOG
		 $(document).ready(function() {
			...
			// calling rss feed from wordpress
			$("#post-list").rss("https://webdevbuddy.wordpress.com/feed/",
			{
				entryTemplate:'<li> ' + 
					'<a class="popup-ajax" href="{url}" target="_blank"> ' + 
						'<h2> ' + 
							'{title} ' + 
						'</h2> ' + 
					'</a> ' + 
					...
					'</p> ' + 
					'<span> ' + 
						'<i class="fa fa-calendar"></i> {longDate} &nbsp;&nbsp;/ ' + 
				...
				'</li>',
				tokens: {
					longDate: function(entry, tokens) { 
						var publishDate = new Date(tokens.date);
						return publishDate.getDate() + ' ' + monthNames[publishDate.getMonth()] + ' ' + publishDate.getFullYear(); 
					}
				}
			});

			// blog list hover effect have to wait a bit due to ajax call have to finish loading
			setTimeout(function () {
				$('.blog ul li').hover(
			     function () {
